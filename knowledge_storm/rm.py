...
class TavilySearchRM(dspy.Retrieve):
    ...
    def __init__(
        ...
        # Creates client instance that will use search
        self.tavily_client = TavilyClient(api_key=self.tavily_search_api_key)
        print(f"Initialized TavilyClient with API key: {self.tavily_search_api_key[:5]}...")
    ...
    def forward(
        ...
        for query in queries:
            try:
                # Using the simplest form of the search API to avoid parameter issues
                print(f"Searching Tavily for: {query[:50]}...")
                responseData = self.tavily_client.search(query=query)
